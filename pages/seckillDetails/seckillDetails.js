...
Page({
    ...
    wx.request({
      ...
      success: function (result) {
        ...
        if (results.success == true) {
          ...
          //将秒杀结束时间转为时间戳并记录
          var timestamp = Date.parse(app.dateFromString(_this.data.shopDetails.enddatetime));
          timestamp = timestamp / 1000;
          _this.setData({
          ...
          let goodsList = [
            { actEndTime: app.dateFromString(_this.data.shopDetails.enddatetime)},
          ]
  ...
  goConfirmOrder: function () {
    ...
    // 判断是否是秒杀商品
    if (this.data.shopDetails.isseckill==true){
      //秒杀商品购买：不需要选择规格，添加isseckill=>true参数
      let temp = [{
      ...
      wx.navigateTo({
        url: '/pages/confirmOrder/confirmOrder?orderArr=' + orderArr + '&isseckill=true',
      })
    }else{
      //普通商品购买：需要选择规格
