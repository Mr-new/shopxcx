...
Page({
  ...
  getOrderMsg: function () {
    ...
    console.log(_this.data.orderArr);
    wx.request({
      url: app.globalData.shopRequestUrl + "Integral/getIntegralOrderMsg",
      data: {
  ...
  goPay:function(){
    ...
    if(_this.data.tel.length==0){
      ...
      let params={  //提交订单数据
        'userid': wx.getStorageSync('userid'),
        'integralgoodsid': _this.data.shopId,
        'tel': _this.data.tel,
        'remarks': _this.data.remarks,
        'sumintegral': _this.data.orderDetails.integral,
      }
      wx.request({
        url: app.globalData.shopRequestUrl + "Integral/submitOrder",
        data: params,
        header: {
          "Content-Type": "application/x-www-form-urlencoded"
        },
        ...
        success: function (result) {
          ...
          if (results.success == true) {
            wx.navigateTo({
              url: '/pages/integralSuccess/integralSuccess?orderNumber=' + results.data['orderNumber'] + '&sumPrice=' + results.data['sumPrice'],
                  })
